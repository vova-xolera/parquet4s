...
object SkippingParquetSchemaResolver
    ...
    new SkippingParquetSchemaResolver[FieldType[K, V] :: T] {
      def resolveSchema(cursor: Cursor): List[Type] = {
        cursor.advance[K] match {
          case Some(newCursor) =>
            newCursor.accept(new TypedSchemaDefInvoker(schemaDef), visitor) +: rest.resolveSchema(cursor)
          case None =>
  ...
  trait SchemaVisitor[K <: Symbol, V] extends Cursor.Visitor[TypedSchemaDefInvoker[K, V], Type] {
    override def onCompleted(cursor: Cursor, invoker: TypedSchemaDefInvoker[K, V]): Type =
  ...
  def defaultSchemaVisitor[K <: Symbol, V]: SchemaVisitor[K, V] = new SchemaVisitor[K, V] {
    override def onActive(cursor: Cursor, invoker: TypedSchemaDefInvoker[K, V]): Type =
      invoker.invoke()
  }
  ...
  implicit def productSchemaVisitor[K <: Symbol, V](implicit resolver: SkippingParquetSchemaResolver[V]): SchemaVisitor[K, V] =
    new SchemaVisitor[K, V] {
      override def onActive(cursor: Cursor, invoker: TypedSchemaDefInvoker[K, V]): Type =
        invoker.invokeOther(typedSchemaDef[V](
          GroupSchemaDef.optional(resolver.resolveSchema(cursor):_*)
        ))
  }
